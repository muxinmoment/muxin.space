---
title: "从手写笔记到个人网站落地：我为什么用 Astro + Cloudflare Pages"
...
## 服务器岛屿值得知道，但不是当前第一优先级

我手写笔记里还记到了服务器岛屿。这个方向确实值得了解，因为它让页面中的某些区域可以由服务端更灵活地单独处理。

不过对我当前这个站来说，它还不是最先要吃透的东西。

原因很简单：我现在网站的主线还是这些偏静态的内容：
...
- Astro 默认怎么做静态输出
- 岛屿架构为什么能减轻页面负担
- 什么时候该上交互，什么时候不该为了“看起来高级”去强行上交互

服务器岛屿可以当成下一阶段的进阶理解，而不是一开始就背进去的重点。
...
### 3. 以为一定要 `wrangler.jsonc`

如果你是：

- `Cloudflare Pages`
- Git 自动部署
- 没有用到 KV、D1、R2 这类绑定资源

那通常不需要单独维护 `wrangler.jsonc`。

`wrangler.jsonc` 更适合下面这些情况：

- Worker 项目
- 命令行手动部署
- 需要显式绑定 Cloudflare 资源

对当前这个站来说，标准的 Pages 配置已经够用了。

### 4. 看到 KV 报错，就以为项目必须接 KV

这次也踩到过 KV namespace 相关报错，但后来回头看，本质问题不是“项目少配了 KV”，而是部署链路选偏了，或者某次自动配置过程误带上了不需要的资源。

一个很实用的判断标准是：先看代码里到底有没有真的使用 KV。如果没有，就不要为了消除某条平台报错，反过来把项目结构绑复杂。
